...
/**
 * Holds domain-specific SSL pinning configuration received from the server or
 * provided as fallback data.
 *
...
data class DomainsConfig(
     ...
     *
     * If the domain is explicitly listed, returns its [DomainConfig.sslPinningRequired] value.
     * Otherwise, falls back to [sslPinningRequiredForUnlisted].
     *
     * @param domain The domain name to check.
     * @return `true` if SSL pinning is required for the given domain.
     */
    fun isPinningRequired(domain: String): Boolean {
        return domains.firstOrNull { it.name == domain }?.sslPinningRequired
            ?: sslPinningRequiredForUnlisted
    }
}

/**
 * Holds SSL pinning configuration for a specific domain.
 *
