...
- The cache structure now includes an optional depth attribute for each stored certificate entry. If this attribute is missing (as in caches created by version 1.5.x), it is assumed to be 0, indicating a leaf certificate. Caches created with earlier versions remain fully compatible with version 1.6.x. No cache reset, server update, or additional action is required.
...
- The `useChallenge` property has been removed from `CertStoreConfiguration`. Challenge-based request signing is now always enabled — the library always sends a random challenge in the `X-Cert-Pinning-Challenge` request header and always validates the ECDSA signature in the `X-Cert-Pinning-Signature` response header.
  - Remove `.useChallenge(...)` from your `CertStoreConfiguration.Builder` call.
  - Ensure your server (e.g. [Mobile Utility Server](https://github.com/wultra/mobile-utility-server)) supports challenge-based response signing. Static JSON data generated by the [SSL Pinning Tool](https://github.com/wultra/ssl-pinning-tool) is no longer supported.
